(function() {
  ...
  _.uniq = function(array, isSorted, iterator = _.identity) {
    ...
    _.each(array, function(element) {
      if (iterator(element) && !result.includes(element)) {
        result.push(element);
      }
    });
  ...
  _.every = function(collection, iterator = _.identity) {
    ...
    return _.reduce(collection, function(result, element) {
      return Boolean(result && iterator(element));
    }, true);
  };
  ...
  _.extend = function(obj) {
    ...
    var additionalObjects = args.slice(1);
  ...
  _.shuffle = function(array) {
    ...
    for (currentIndex = copy.length - 1; currentIndex > 0; currentIndex--) {
      randomIndex = Math.floor(Math.random() * (currentIndex + 1));
      currentVal = copy[currentIndex];
